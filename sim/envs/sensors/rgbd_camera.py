...
class Camera:
    ...
    def __init__(
        ...
        model=None,
        data=None,
        cam_name: str = "",  # camera must be inserted on the .xml file of the robot in order to work
        save_dir="data/img/",
    ):
        ...
        self._last_depth_image = np.zeros(
            (self._height, self._width, 1), dtype=np.float32
        )
        self._last_seg_id_image = np.zeros(
            (self._height, self._width, 3), dtype=np.float32
        )
        self._last_point_cloud = np.zeros(
            (self._height, self._width, 1), dtype=np.float32
        )
    ...
    def save(
        ...
        if img_name == "":
            ...
            if seg:
                cv2.imwrite(
                    self._save_dir + f"seg_{self.save_counter}.png", self.seg_image
                )
            if depth:
                np.save(
                    self._save_dir + "/images/" + f"depth_{self.save_counter}.npy",
                    self.depth_image,
                )
            self.save_counter += 1
        else:
            ...
            if depth:
                np.save(
                    self._save_dir + "/images/" + f"{img_name}_depth.npy",
                    self.depth_image,
                )
